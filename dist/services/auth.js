...
export function logout() {
    fs.writeFileSync(process.env._PORTEX_CONFIG_TOKEN_FILE_PATH_, '');
}
export function isLoggedIn() {
    try {
        const token = fs.readFileSync(process.env._PORTEX_CONFIG_TOKEN_FILE_PATH_, 'utf8').trim();
...
export function getToken() {
    try {
        return fs.readFileSync(process.env._PORTEX_CONFIG_TOKEN_FILE_PATH_, 'utf8').trim();
    }
    catch {
        return '';
    }
