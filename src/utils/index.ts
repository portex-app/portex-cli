...
export const writeLogFile = (content: string): void => {
    try {
        ...
        const logFilePath = path.join(process.env._PORTEX_CONFIG_DIR_ as string, 'log');
 ...
 */
export const getTableConfig = (columnsConfig: Indexable<ColumnUserConfig>, spanningCells: SpanningCellConfig[] = []): TableUserConfig => ({
    // 定义表格边框样式
    border: {
        bodyJoin: `│`,
        bodyLeft: `│`,
