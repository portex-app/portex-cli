...
async function resolveAppName(argName?: string): Promise<string> {
    ...
    if (!name) fail(t('error.appNotFound'))
    return name
}
...
export async function runCLI(argv: string[]): Promise<void> {
    ...
    program
        ...
        .action(async (opts) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))

            const name = opts.name ?? await prompt(`${t('new.appName')}: `)
            const nameRegex = /^[a-z0-9_]{5,35}$/
            if (!nameRegex.test(name)) fail(t('new.appNameInvalid'))
            ...
            if (opts.platform) {
                ...
                if (idx < 0 || idx >= platforms.length) fail(t('error.platformNotFound'))
    ...
    program
        ...
        .action(async (opts) => {
            ...
            const params: ApplicationQueryRequest = { application_id: opts.id }
            if (opts.platform) params.platform_id = await getPlatformByName(opts.platform)

            let apps = await listApplications(params)
            if (!apps.length) { info(t('list.empty')); return }
            ...
            console.log(line)
            console.log(
                col(t('list.headers.name'), 20),
                col(t('list.headers.platform'), 12),
                col(t('list.headers.lastVersion'), 10),
                col(t('list.headers.dev'), 6),
                col(t('list.headers.test'), 6),
                col(t('list.headers.prod'), 6),
            ...
            for (const app of pageApps) {
                ...
                console.log(
                    col(app.name, 20),
                    col(app.platform_name, 12),
                    col(String(app.last_version), 10),
                    col(vmap['0'] ?? '-', 6),
                    col(vmap['1'] ?? '-', 6),
                    col(vmap['2'] ?? '-', 6),
                    app.description ?? '-'
    ...
    program
        ...
        .action(async (argName?: string, argPath?: string, argDesc?: string) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))
    ...
    program
        ...
        .action(async (argName?: string, argVersion?: string, opts?: { env: string }) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))
            ...
            if (argVersion) {
                ...
                if (!versions.length) fail(t('publish.noVersions'))
    ...
    bot
        ...
        .action(async (appName: string, botToken: string) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))
            const app = await getApplicationInfo({ application_name: appName })
            if (app.platform_name !== 'Telegram') fail(t('bot.onlyTelegram'))
    ...
    bot
        ...
        .action(async (appName: string, filePath?: string, opts?: { input?: boolean; output?: boolean; template?: boolean }) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))
            const app = await getApplicationInfo({ application_name: appName })
            if (app.platform_name !== 'Telegram') fail(t('bot.onlyTelegram'))
    ...
    botMenu
        ...
        .action(async (appName: string, menuName: string, menuURL: string) => {
            if (!isLoggedIn()) fail(t('error.unauthorized'))
            const app = await getApplicationInfo({ application_name: appName })
            if (app.platform_name !== 'Telegram') fail(t('bot.onlyTelegram'))
