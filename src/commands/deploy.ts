...
export default class Deploy extends BaseCommand {
     ...
     * @type {string}
     */
    static override description: string = 'deploy mini-app, upload mini-app package from application build path.';
    static override summary: string = 'Deploy mini-app';
     ...
     */
    private async clearOldFile(): Promise<void> {
        const packagePath = process.env._PORTEX_CONFIG_PACKAGE_FILE_PATH_ as string;
        const files = fs.readdirSync(packagePath);
        const fileMap: { createTime: number; fileName: string; }[] = [];
     ...
     * @returns {Promise<void>}
     */
    private async uploadFile(presignedUrl: string, zipFilePath: string, fileMD5: string, description: string): Promise<void> {
