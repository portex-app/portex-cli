...
export default class BotMessage extends BaseCommand {
     ...
     */
    static override description = 'get or save bot messages';
    /**
     ...
     */
    static override summary = 'Get or save bot messages';
    /**
     ...
     */
    private async getBotMessages(application_id: string, save_file_path: string, isTemp: boolean = false): Promise<void> {
        try {
            let commands;
            if (isTemp) {
     ...
     */
    private getDefaultMessageTemp(): BotMessages {
        return {
            "buttons_message": {
                "buttons": [
                    {
